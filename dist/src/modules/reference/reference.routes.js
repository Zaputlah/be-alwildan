...
async function validateClassRelations(schoolUnitId, academicYear, homeroomTeacherId) {
    if (academicYear) {
        const period = await prisma.academicPeriod.findFirst({ where: { name: academicYear, schoolUnitId } });
        if (!period)
            throw new HttpError(400, 'INVALID_YEAR', 'Tahun ajaran tidak tersedia pada unit sekolah Anda.');
    }
    if (homeroomTeacherId) {
        const teacher = await prisma.user.findFirst({ where: { id: homeroomTeacherId, schoolUnitId, role: 'TEACHER', isActive: true } });
        if (!teacher)
            throw new HttpError(400, 'INVALID_HOMEROOM_TEACHER', 'Wali kelas harus merupakan Guru aktif pada unit sekolah ini.');
    }
}
...
referenceRouter.patch('/classes/:id', authorize('ADMIN'), requireCsrf, validate(classSchema.partial()), async (req, res) => {
    ...
    if (!existing)
        throw new HttpError(404, 'CLASS_NOT_FOUND', 'Kelas tidak ditemukan.');
    await validateClassRelations(req.auth.schoolUnitId, req.body.academicYear, req.body.homeroomTeacherId);
...
referenceRouter.delete('/classes/:id', authorize('ADMIN'), requireCsrf, async (req, res) => {
    ...
    if (usage > 0)
        throw new HttpError(409, 'CLASS_IN_USE', 'Kelas masih memiliki siswa, penugasan, atau assessment sehingga tidak dapat dihapus.');
...
referenceRouter.patch('/subjects/:id', authorize('ADMIN'), requireCsrf, validate(subjectSchema.partial()), async (req, res) => {
    ...
    if (!existing)
        throw new HttpError(404, 'SUBJECT_NOT_FOUND', 'Mata pelajaran tidak ditemukan.');
...
referenceRouter.delete('/teaching-assignments/:id', authorize('ADMIN'), requireCsrf, async (req, res) => {
    ...
    if (!assignment)
        throw new HttpError(404, 'ASSIGNMENT_NOT_FOUND', 'Penugasan guru tidak ditemukan.');
    ...
    if (activeAssessment > 0)
        throw new HttpError(409, 'ASSIGNMENT_HAS_DRAFT', 'Selesaikan atau hapus assessment draft sebelum mencabut penugasan.');
