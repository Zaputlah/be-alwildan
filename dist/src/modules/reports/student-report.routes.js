...
async function reportScope(req, classId, academicPeriodId) {
    ...
    if (!assignments.length)
        throw new HttpError(403, 'NOT_ASSIGNED', 'Anda tidak dapat melihat rekap kelas ini.');
...
studentReportRouter.get('/students', async (req, res) => {
    ...
    if (!parsed.success)
        throw new HttpError(400, 'VALIDATION_ERROR', 'Pilih kelas dan semester.', parsed.error.flatten());
...
studentReportRouter.get('/students/:studentId', async (req, res) => {
    ...
    if (!student)
        throw new HttpError(404, 'STUDENT_NOT_FOUND', 'Siswa tidak ditemukan pada kelas dan semester ini.');
