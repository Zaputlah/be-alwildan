...
async function attendanceScope(classId, academicPeriodId, schoolUnitId, teacherId) {
    ...
    if (teacherId && schoolClass.homeroomTeacherId !== teacherId) {
        ...
        if (!assigned)
            throw new HttpError(403, 'NOT_ASSIGNED', 'Anda tidak dapat melihat absensi kelas ini.');
    }
...
studentAttendanceRouter.get('/', async (req, res) => {
    ...
    if (!parsed.success)
        throw new HttpError(400, 'VALIDATION_ERROR', 'Filter absensi siswa tidak valid.', parsed.error.flatten());
...
studentAttendanceRouter.put('/records', authorize('ADMIN', 'TEACHER'), requireCsrf, validate(saveSchema), async (req, res) => {
    ...
    if (new Set(studentIds).size !== studentIds.length)
        throw new HttpError(400, 'DUPLICATE_STUDENT', 'Siswa tidak boleh dicatat dua kali pada tanggal yang sama.');
    ...
    if (enrolled !== studentIds.length)
        throw new HttpError(400, 'INVALID_STUDENT', 'Ada siswa yang bukan anggota aktif kelas dan semester ini.');
...
studentAttendanceRouter.patch('/records/:studentId/late', authorize('TEACHER'), requireCsrf, validate(markLateSchema), async (req, res) => {
    ...
    if (!existing)
        throw new HttpError(404, 'ATTENDANCE_NOT_FOUND', 'Absensi siswa belum dicatat pada tanggal ini.');
    if (existing.status !== 'ABSENT')
        throw new HttpError(409, 'ATTENDANCE_LOCKED', 'Hanya siswa berstatus Alpa yang dapat ditandai datang terlambat.');
    ...
    if (!updated.count)
        throw new HttpError(409, 'ATTENDANCE_LOCKED', 'Absensi sudah berubah. Muat ulang halaman sebelum mencoba lagi.');
