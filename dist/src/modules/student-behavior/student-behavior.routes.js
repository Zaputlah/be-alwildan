...
async function behaviorScope(classId, academicPeriodId, schoolUnitId, teacherId) {
    ...
    if (teacherId && !isHomeroom) {
        ...
        if (!assigned)
            throw new HttpError(403, 'NOT_ASSIGNED', 'Anda tidak dapat mengakses catatan perilaku kelas ini.');
    }
...
studentBehaviorRouter.get('/', async (req, res) => {
    ...
    if (!parsed.success)
        throw new HttpError(400, 'VALIDATION_ERROR', 'Filter perilaku siswa tidak valid.', parsed.error.flatten());
...
studentBehaviorRouter.put('/records', requireCsrf, validate(saveSchema), async (req, res) => {
    ...
    if (!student)
        throw new HttpError(400, 'INVALID_STUDENT', 'Siswa bukan anggota aktif kelas dan semester ini.');
