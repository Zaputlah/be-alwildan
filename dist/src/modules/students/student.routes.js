...
async function nextNis(tx, schoolUnitId, prefix) {
...
studentRouter.get('/next-nis', authorize('ADMIN'), async (req, res) => {
    ...
    if (!parsed.success)
        throw new HttpError(400, 'VALIDATION_ERROR', 'Pilih kelas terlebih dahulu.');
    const schoolClass = await prisma.schoolClass.findFirst({
        where: { id: parsed.data.classId, schoolUnitId: req.auth.schoolUnitId },
        select: { name: true, gradeLevel: true },
    });
    if (!schoolClass)
        throw new HttpError(400, 'INVALID_CLASS', 'Kelas tidak tersedia pada unit sekolah Anda.');
...
studentRouter.patch('/:id', authorize('ADMIN'), requireCsrf, validate(studentSchema.partial()), async (req, res) => {
    ...
    if (!existing)
        throw new HttpError(404, 'STUDENT_NOT_FOUND', 'Siswa tidak ditemukan.');
    const { classId, academicPeriodId, birthDate, ...fields } = req.body;
    ...
    if (classId && !targetClass)
        throw new HttpError(400, 'INVALID_CLASS', 'Kelas tidak valid.');
