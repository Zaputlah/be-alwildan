...
function isEarlyCheckout(checkOutAt) {
...
teacherAttendanceRouter.get('/', async (req, res) => {
    ...
    if (!parsed.success)
        throw new HttpError(400, 'VALIDATION_ERROR', 'Filter absensi tidak valid.', parsed.error.flatten());
    ...
    if (teacherId) {
        ...
        if (!teacher)
            throw new HttpError(400, 'INVALID_TEACHER', 'Guru tidak tersedia pada unit sekolah Anda.');
    }
    ...
    for (const record of records) {
        if (countsAsFinalAttendance(record.approvalStatus))
...
teacherAttendanceRouter.post('/check-in', authorize('TEACHER'), requireCsrf, validate(clockSchema), async (req, res) => {
    ...
    if (existing?.checkInAt)
        throw new HttpError(409, 'ALREADY_CHECKED_IN', 'Anda sudah melakukan check-in hari ini.');
...
teacherAttendanceRouter.post('/check-out', authorize('TEACHER'), requireCsrf, validate(clockSchema), async (req, res) => {
    ...
    if (!existing?.checkInAt)
        throw new HttpError(409, 'NOT_CHECKED_IN', 'Lakukan check-in terlebih dahulu.');
    if (existing.checkOutAt)
        throw new HttpError(409, 'ALREADY_CHECKED_OUT', 'Anda sudah melakukan check-out hari ini.');
    const location = req.body.location;
...
teacherAttendanceRouter.post('/requests', authorize('TEACHER'), requireCsrf, uploadEvidence, async (req, res) => {
    ...
    if (!parsed.success)
        throw new HttpError(400, 'VALIDATION_ERROR', 'Data sakit/izin/tugas tidak valid.', parsed.error.flatten());
...
teacherAttendanceRouter.patch('/requests/:id/review', authorize('ADMIN'), requireCsrf, validate(reviewSchema), async (req, res) => {
    ...
    if (!record)
        throw new HttpError(404, 'REQUEST_NOT_PENDING', 'Pengajuan tidak ditemukan atau sudah diproses.');
    ...
    if (!updated.count)
        throw new HttpError(409, 'REQUEST_ALREADY_REVIEWED', 'Pengajuan sudah diproses oleh Admin lain.');
...
teacherAttendanceRouter.get('/records/:id/evidence', async (req, res) => {
    ...
    if (!record?.evidence)
        throw new HttpError(404, 'EVIDENCE_NOT_FOUND', 'Bukti absensi tidak ditemukan.');
...
teacherAttendanceRouter.put('/records', authorize('ADMIN'), requireCsrf, uploadEvidence, async (req, res) => {
    let input;
    try {
        input = JSON.parse(String(req.body.data ?? ''));
    }
    catch {
        throw new HttpError(400, 'VALIDATION_ERROR', 'Data absensi tidak valid.');
    }
    const parsed = manualRecordSchema.safeParse(input);
    if (!parsed.success)
        throw new HttpError(400, 'VALIDATION_ERROR', 'Data absensi tidak valid.', parsed.error.flatten());
...
teacherAttendanceRouter.delete('/records/:id', authorize('ADMIN'), requireCsrf, async (req, res) => {
    ...
    if (!record)
        throw new HttpError(404, 'ATTENDANCE_NOT_FOUND', 'Data absensi tidak ditemukan.');
