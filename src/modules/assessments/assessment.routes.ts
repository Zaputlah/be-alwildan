...
async function canManage(userId: string, role: string, schoolUnitId: string, classId: string, subjectId: string, academicPeriodId: string) {
  ...
  if (!schoolClass || !period || schoolClass.academicYear !== period.name) return false;
  ...
  const teacherVisibility = req.auth!.role === 'TEACHER'
    ? {
        OR: [
          ...
          { status: 'PUBLISHED' as const, schoolClass: { homeroomTeacherId: req.auth!.userId } },
        ],
      }
    : {};
...
assessmentRouter.post('/', requireCsrf, validate(assessmentSchema), async (req, res) => {
  const data = req.body as z.infer<typeof assessmentSchema>;
...
assessmentRouter.get('/:id/scores', async (req, res) => {
  ...
  if (!assessment) throw new HttpError(404, 'ASSESSMENT_NOT_FOUND', 'Assessment tidak ditemukan.');
...
assessmentRouter.put('/:id/scores', requireCsrf, validate(scoreSchema), async (req, res) => {
  ...
  if (assessment.status === 'PUBLISHED') throw new HttpError(409, 'ALREADY_PUBLISHED', 'Nilai yang sudah diterbitkan tidak dapat diubah.');
  const scores = (req.body as z.infer<typeof scoreSchema>).scores;
