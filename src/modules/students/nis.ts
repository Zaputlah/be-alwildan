...
export function nisPrefixForClass(schoolClass: { name: string; gradeLevel: number }): string {
  const match = /^([1-9]|1[0-2])([A-I])$/.exec(schoolClass.name.trim().toUpperCase());
  if (!match || Number(match[1]) !== schoolClass.gradeLevel) {
    throw new HttpError(
      400,
      'INVALID_CLASS_FOR_NIS',
      'Nama kelas harus mengikuti format tingkat dan paralel A-I, misalnya 7A atau 8B.',
    );
  }
  return `${schoolClass.gradeLevel}${match[2].charCodeAt(0) - 64}`;
}

export function formatNis(prefix: string, serial: number): string {
