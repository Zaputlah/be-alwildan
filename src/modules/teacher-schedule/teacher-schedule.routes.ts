...
teacherScheduleRouter.get('/', async (req, res) => {
  ...
  if (requestedPeriodId && !period) throw new HttpError(404, 'PERIOD_NOT_FOUND', 'Semester tidak ditemukan pada unit sekolah ini.');
...
async function ensureSlotAllowed(input: z.infer<typeof slotSchema>, schoolUnitId: string, excludeId?: string) {
  ...
  if (!assignment) throw new HttpError(400, 'INVALID_ASSIGNMENT', 'Penugasan guru tidak ditemukan pada unit sekolah ini.');
  ...
  if (conflicting) throw new HttpError(409, 'SCHEDULE_CONFLICT', 'Jadwal bentrok dengan jam mengajar guru atau kelas tersebut.');
}
...
teacherScheduleRouter.put('/:id', authorize('ADMIN'), requireCsrf, validate(slotSchema), async (req, res) => {
  ...
  if (!existing) throw new HttpError(404, 'SCHEDULE_NOT_FOUND', 'Jadwal tidak ditemukan.');
